...
// const uint32_t qdcAds[nQDCs] = { 0x1B110000, 0x06000000, 0x05000000, 0x03000000 }; // debug addressing 2024.09.05
...
void initV792( v792ac* v792adc )
 {
  ...
  cout << " 1 initV792 " << endl;
  v792adc->clearData();
  cout << " 2 initV792 " << endl;
  uint16_t ped;
  ped = v792adc->getI1();
  cout << " 3 initV792 " << endl;
...
void initV775N( v775n& xtdc )
 {
  ...
  // xtdc.setLSB(V775_140ps);   // set the LSB (ps)  full scale = 4096*LSB ==> 140 ps * 4096 ~ 573 ns
  xtdc.setLSB(V775_100ps);   // set the LSB (ps)  full scale = 4096*LSB ==> 100 ps * 4096 ~ 410 ns
  xtdc.storeEmpty();
  xtdc.evCntReset();
  xtdc.dataReset();
  xtdc.printRegisters();
    ...
    do
     {
      ...
      if (abort_run) return 0xffff0000;
     }
    while (!dr);
...
void print_event ( ofstream& ofs, uint32_t trignum, struct timeval tod,  uint32_t spillNr, uint32_t* Tcts, uint32_t tmask, uint32_t* myBuffer, uint32_t qdcsize, uint32_t tdcsize )
 {
  struct tm * timeinfo = localtime ( &tod.tv_sec );
  ofs << dec << " ev # " << trignum << " time " << timeinfo->tm_yday
     << "-" << timeinfo->tm_hour << "-" << timeinfo->tm_min << "-"
     << timeinfo->tm_sec << "-" << tod.tv_usec << " spill " << spillNr
...
void print_fake_event ( ofstream& ofs, uint32_t trignum, struct timeval tod,  uint32_t spillNr, uint32_t* Tcts, uint32_t* myBuffer )
 {
  struct tm * timeinfo = localtime ( &tod.tv_sec );
  ofs << dec << " ev # " << trignum << " time " << timeinfo->tm_yday
     << "-" << timeinfo->tm_hour << "-" << timeinfo->tm_min << "-"
...
int main( int argc, char** argv )
{
  ...
  cout << " ****************** start of run " << runnbr << " ****************** " << endl << endl;
  ...
  cout << " mySem val is " << mySem.semGet() << endl;
  ...
  for (int ccc = 0; true; ccc++)
   {
    hereIam = 1;
    isSpill = ((ioreg.readInputRegister() & 0x8000) != 0);
    // if ((ccc % 10000) == 1) cout << dec << ccc << " spillNr " << spillNr << " isSpill " << isSpill << " wasSpill " << wasSpill << endl;
    if (running and pause_run)
     {
      ...
      uint32_t inpval;
      v2718_0.readInputRegister ( &inpval );
      cout << dec << tr << "(sec) " << trignum << "(evt) ioreg A " << hex << inpval
    ...
    if (!isSpill)
      ...
      if (isSpill != wasSpill)
       {
        ...
        time_t tr = time(NULL) - tr0;
        cout << " END of spill: " << dec << spillNr << " at " << tr << "(sec) " << triginspill << "(evt) "
	       << "beam/pede/inSpill/beamInSpill: "
	       << nowCts[0] << "/" << nowCts[1] << "/" << nowCts[2] << "/" << nowCts[3]
	       << " Total beam/pede/inSpill/beamInSpill: "
	       << Tcts[0] << "/" << Tcts[1] << "/" << Tcts[2] << "/" << Tcts[3] << endl;
        prevTrig = trignum;
        prevCts[0] = Tcts[0];
        prevCts[1] = Tcts[1];
        prevCts[2] = Tcts[2];
     ...
     {
      hereIam = 4;
      // cout << " 4 isSpill " << isSpill << " wasSpill " << wasSpill << endl;
      if (isSpill != wasSpill)
       {
        spillNr ++;
        wasSpill = true;
      ...
      // cout << " 5 isSpill " << isSpill << " wasSpill " << wasSpill << " isTrig " << isTrig << endl;
      ...
      if (isTrig)
       {
        ...
        hereIam = 8;
        if (1) print_event ( ofs, trignum, tode, spillNr, Tcts, tmask_a, myBuffer, qdcsize, tdcsize );
        // while (trignum <= evnum) trignum ++; // evt_realign ( v2718_0, qdc, Tcts, ofs, trignum );
        trignum ++;

        if (retcode == 0xff)     // spurious trigger to all elx -- add one fake event to align VME data with FERS data
         {
          print_fake_event ( ofs, trignum, tode, spillNr, Tcts, myBuffer );
          trignum ++;
         }
        ...
        unlockTrigger( v2718_0 );
        // cout << " 6 isSpill " << isSpill << " wasSpill " << wasSpill << " isTrig " << isTrig << endl;
       }
      hereIam = 11;
     }
    volatile int sv = mySem.semGet();
    // usleep(500000);
    if (0) cout << ccc << " evt " << trignum << " mySem.semGet() is " << sv << endl;
    if (abort_run || (sv > 0))
     {
      cout << dec << " exiting now after " << trignum << " events - semaphore " << sv << " abort_run " << abort_run << endl;
      break;
     }
   }
  ...
  cout << " ****************** end of run " << runnbr << " ****************** " << endl << endl;
