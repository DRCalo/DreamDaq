...
const uint32_t nQDC = 4;
const uint32_t QDCchans = 128;
...
uint32_t resetHardware ( v792ac& qdc0, v792ac& qdc1, v792ac& qdc2, v792ac& qdc3, v775n& xtdc0 )
 {
  initV792 ( qdc0 );
  sched_yield();
...
uint32_t alignEventCounter ( uint32_t evtnumber, v792ac& qdc0, v792ac& qdc1, v792ac& qdc2, v792ac& qdc3, v775n& xtdc0 )
 {
  qdc0.eventCounterOffset ( evtnumber );
...
uint32_t readEvent ( v792ac& qdc0, v792ac& qdc1, v792ac& qdc2, v792ac& qdc3, v775n& xtdc0, uint32_t ntriggers, uint32_t* buffer, uint32_t* qdcsize, uint32_t* tdcsize )
 {
  ...
  buffer += sizetdc;
  *qdcsize = size0+size1+size2+size3;
  *tdcsize = sizetdc;
  uint32_t nevqdc0, nevqdc1, nevqdc2, nevqdc3, nevtdc0;
  ...
  uint32_t errcode = (ntriggers != nevqdc0) | (ntriggers != nevqdc1) << 1
	| (ntriggers != nevqdc2) << 2 | (ntriggers != nevqdc3) << 3 | (ntriggers != nevtdc0) << 4;
  if ((errcode == 31) && (nevqdc0 == ntriggers+1)) errcode = 0xff;
  if (errcode)
   {
    ...
    cerr << dec << ntriggers << " nq0 " << nevqdc0 << " nq1 " << nevqdc1 << " nq2 " << nevqdc2 << " nq3 " << nevqdc3 << " nt0 " << nevtdc0 << " - error code " << hex << errcode << endl;
   } 
  if (size0*size1*size2*size3 == 0)
   {
    errcode |= 1 << 7;
    time_t rtime = time(NULL);
    struct tm myrtime;
    ...
    cerr << dec << ntriggers << " size0 " << size0 << " size1 " << size1 << " size2 " << size2 << " size3 " << size3 << " sizetdc " << sizetdc << " - error code " << hex << errcode << endl;
   }
...
int main( int argc, char** argv )
{
  ...
  v862ac qdc0(0x03000000,"/V2718/cvA32_U_DATA/0");      // V862 QDC
  v792ac qdc1(0x04000000,"/V2718/cvA32_U_DATA/0");      // V792 QDC
  v792ac qdc2(0x05000000,"/V2718/cvA32_U_DATA/0");      // V792 QDC
  v792ac qdc3(0x06000000,"/V2718/cvA32_U_DATA/0");      // V792 QDC
  ...
  // Init QDCs and TDCs
  resetHardware ( qdc0, qdc1, qdc2, qdc3, xtdc0 );
  ...
  while (1)
  {
    ...
    if (isPh) do 
    {
      ...
      uint32_t retcode = readEvent ( qdc0, qdc1, qdc2, qdc3, xtdc0, trignum, myBuffer, &qdcsize, &tdcsize );
	...
	cerr << dec << trignum << " Size error " << qdcsize << endl;
      ...
      if (on_error)
      {
        ...
        cerr << myrtime.tm_year+1900 << "." << myrtime.tm_mon+1 << "." << myrtime.tm_mday << "." 
             << myrtime.tm_hour << ":" << myrtime.tm_min << ":" << myrtime.tm_sec << endl;
        cerr << asctime(&myrtime) << endl;

        tmask |= 0xffff0000;
        resetHardware ( qdc0, qdc1, qdc2, qdc3, xtdc0 );
        myusleep ( 100000 );
        alignEventCounter ( trignum, qdc0, qdc1, qdc2, qdc3, xtdc0 );
        numevofs = trignum;
        myusleep ( 100000 );
