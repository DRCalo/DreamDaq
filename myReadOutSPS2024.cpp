...
uint32_t readEvent ( v792ac& qdc0, v792ac& qdc1, v792ac& qdc2, v775n& xtdc0, uint32_t ntriggers, uint32_t* buffer, uint32_t* qdcsize, uint32_t* tdcsize )
 {
  nsleep(NSLEEPT);
  ...
  buffer += sizetdc;
  *qdcsize = size0+size1+size2;
  *tdcsize = sizetdc;
  uint32_t nevqdc0, nevqdc1, nevqdc2, nevtdc0;
  ...
  uint32_t errcode = (ntriggers != nevqdc0) | (ntriggers != nevqdc1) << 1
	| (ntriggers != nevqdc2) << 2 | (ntriggers != nevtdc0) << 3;
  if (errcode) cerr << dec << ntriggers << " nq0 " << nevqdc0 << " nq1 " << nevqdc1 << " nq2 " << nevqdc2 << " nt0 " << nevtdc0 << " - error code " << hex << errcode << endl;
  if (size0*size1*size2 == 0)
   {
    errcode |= 1 << 7;
    cerr << dec << ntriggers << " size0 " << size0 << " size1 " << size1 << " size2 " << size2 << " sizetdc " << sizetdc << " - error code " << hex << errcode << endl;
   }
  return errcode;
 }
...
if ((trignum % 10000) == 0)
 {
  ...
  cout << dec << tr << "(sec) " << trignum << "(evt) "
      ...
      uint32_t retcode = readEvent ( qdc0, qdc1, qdc2, xtdc0, trignum, myBuffer, &qdcsize, &tdcsize );
